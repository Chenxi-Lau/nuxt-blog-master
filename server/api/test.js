...
router.post('/', function (req, res) {
  ...
  function selectUser (id) {
    pool.query(`SELECT * FROM user_info where userId=${id}`, function (err, results) {
      if (err) {
        data.code = 500
        data.msg = err
      }
...
// 获取最近一周
router.get('/tripTrend', function (req, res) {
  let tmpId = req.query.userId
  // 增删改查之后查询，并把查询的最终数据返回前端

  pool.query(`SELECT * FROM trip_data where userId=${tmpId} and DATE_SUB(CURDATE(), INTERVAL 6 DAY) <= date(date)`, function (err, results) {
    if (err) {
      data.code = 500
      data.msg = err
    }
